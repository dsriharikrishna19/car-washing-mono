...
export const getPartnerBookings = catchAsync(async (req, res) => {
    if (!req.user)
        throw new ApiError(401, 'Unauthorized');
...
export const assignPartner = catchAsync(async (req, res) => {
    const booking = await bookingService.assignPartner(req.params.bookingId, req.body.partnerId);
    res.send(booking);
});
